...
namespace React.Server.Controllers
{
    ...
    public class WebSocketController : ControllerBase
    {
        ...
        public WebSocketController(MyOptions options)
        {
        ...
        public async Task Get(int id)
        {
            if (HttpContext.WebSockets.IsWebSocketRequest)
            {
                ...
                var webSocketManager = new React.Server.WebSocketManager(webSocket, _options);
                //await webSocketManager.KeepAlive(id);
                await webSocketManager.HandleWebSocketConnection(id);
                //await BeginScenary(id, webSocket);
            }
            else
            {
        ...
        //    for (int i = 0; i < 10; i++)
        //    {
        //        var serverMsg = Encoding.UTF8.GetBytes($"{i}");
        //        await webSocket.SendAsync(new ArraySegment<byte>(serverMsg, 0, serverMsg.Length), WebSocketMessageType.Text, true, CancellationToken.None);
